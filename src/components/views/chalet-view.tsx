...
/** Page d'atterrissage « chef privé chalet Tremblant » (audit SEO). */
export function ChaletView({ locale }: { locale: Locale }) {
  const t = getDictionary(locale).chaletPage;
  const url = `${siteUrl}${path(locale, "chaletChef")}`;
  ...
  return (
    <>
      ...
      <PageHero
        ...
        image="/service-chef-a-domicile.jpg"
      />
      ...
      <section className="bg-cream px-5 py-20 lg:py-28">
        <div className="mx-auto max-w-3xl text-center">
          <Reveal>
            ...
            <div className="mt-8">
              <ButtonLink
                href={menuPath(locale, "chef-a-domicile")}
                variant="olive"
              >
                {t.menuCta}
              </ButtonLink>
