...
/** Page d'atterrissage « traiteur corporatif Laurentides » (audit SEO). */
export function CorporateView({ locale }: { locale: Locale }) {
  const t = getDictionary(locale).corporatePage;
  const url = `${siteUrl}${path(locale, "corporateCatering")}`;
  ...
  return (
    <>
      ...
      <PageHero
        ...
        image="/service-traiteur-evenements.jpg"
      />
            ...
            <div className="mt-14 grid gap-10 md:grid-cols-3">
              {t.formats.map((f, i) => (
                <div key={f.title} className="text-center">
                  ...
                  <h3 className="mt-5 text-2xl text-olive-dark">{f.title}</h3>
                  <p className="mt-3 leading-relaxed text-ink/85">{f.body}</p>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      <section className="bg-cream px-5 py-20 lg:py-28">
        <div className="mx-auto max-w-3xl text-center">
          <Reveal>
            ...
            <div className="mt-8">
              <ButtonLink
                href={menuPath(locale, "traiteur-evenements")}
                variant="olive"
              >
                {t.menuCta}
              </ButtonLink>
